...
public interface ExtAxisVanMapper {

    @Insert("INSERT INTO AxisBankVanAccount (bankName, bankLock, account, ifsc, account_name, status, van_id, van_type, createAt, updateLock) VALUES "
            + "(#{bankName}, #{bankLock}, #{account}, #{ifsc}, #{account_name}, #{status}, #{van_id}, #{van_type}, #{createAt}, #{updateLock});")
    @SelectKey(keyProperty = "ippoVanId", resultType = Long.class, before = false, statement = "SELECT LAST_INSERT_ID()")
    int addAxisVanAccount(Map<String, Object> body);

    @Insert("INSERT INTO AxisBankVanOrderRecord (platformId, userId, gameId,payConfigId, channelId, status,createTime) VALUES "
    ...
    int addAxisVanOrderRecord(Map<String, Object> body);

    @Update("UPDATE AxisBankVanAccount SET  bankLock=#{bankLock},updateLock=now() WHERE bankName=#{bankName}  and bankLock=0 limit 1")
    int lockAxisVanAccount(@Param("bankLock") Long id, @Param("bankName") String bankName);

    @Select("select * from AxisBankVanAccount where  bankLock  =#{bankLock} ")
    Map<String, Object> getAxisVanAccount(@Param("bankLock") Long id);

    @Select("SELECT * FROM  AxisBankVanOrderRecord  where platformId=#{platformId} and userId=#{userId} and channelId=#{channelId} and status=1 order  by  id desc  limit 1")
    Map<String, Object> countAxisVanOrderRecord(@Param("platformId") Integer platformId,
    ...
    @Select("SELECT * FROM AxisBankVanAccount WHERE account = #{account}")
    List<Map<String, Object>> listAxisVanAccountByAccountAndIfsc(@Param("account") String account);

    @Select("SELECT * FROM AxisBankVanOrderRecord WHERE id = #{id}")
    Map<String, Object> getAxisVanOrderRecord(@Param("id") Long id);
    ...
    @Update("UPDATE AxisBankVanOrderRecord SET ippoVanId =#{ippoVanId}, van_id =#{van_id}, account = #{account}, ifsc =#{ifsc},account_name=#{account_name}, status =#{status},successTime=#{successTime},bankName=#{bankName} WHERE id = #{id}")
    int updateBankVanOrderRecord2(Map<String, Object> body);

    @Update("UPDATE AxisBankVanOrderRecord SET  successTime=#{successTime} WHERE id = #{id}")
    int updateAxisVanOrderRecord3(Map<String, Object> body);
