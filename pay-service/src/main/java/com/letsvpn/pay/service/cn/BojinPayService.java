...
@Service
@Slf4j
public class BojinPayService extends BaseThirdService implements IPayThirdRequest, IPayNotifyHandle {
...
//			result.setTest(true);
		...
		try {
			...
			HttpRequest hr = HttpRequest.post(reqLink).form(formData1)
					.timeout(ThirdUtil.HTTP_REQUEST_TIMEOUT);// 超时，毫秒

//			if (PayHttpType.raw.equals(httpType)) {// json格式请求
...
//			}
			...
			add(merchantInfo.getPlatformId(), payConfigId, channel.getId(), channel.getTitle(), e.getMessage(),
					resultText, this.getClass().getSimpleName());
			if (payConfigInfo.getTest() == 1) {
				throw e;
			} else {
